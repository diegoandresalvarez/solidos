%% Torsion de una barra con seccion transversal cuadrada
E  = 21000000;     % kPa = 21GPa - m�dulo de Young del acero
nu = 0.28;         % coeficiente de Poisson del acero
G  = E/(2*(1+nu)); % modulo de cortante
L  = 1;            % m
rho_L = 0.05;      % rad
theta = rho_L/L;   % rad/m
delta = 0.001;    % espaciado de la malla (metros)
borde  = ... % coordenadas del borde (x,y) que forman el perimetro
[-0.01  -0.01
  0.01  -0.01
  0.01   0.01
 -0.01   0.01
 -0.01  -0.01 ]; %observe que se repite la primera fila

% Especifique un punto (x,y) dentro del s�lido (semilla para la maximizaci�n)
xden = 0;
yden = 0;
...
%% Aqu� defino las coordenas en x e y que estoy manejando
x = -0.01:delta:0.01;
y = -0.01:delta:0.01;
...
title({'Función de tension de Prandtl \phi','(solucion numerica)'},'FontSize', 18) % Escribir el titulo
% Defino las coordenadas que quiero visualizar [xmin xmax ymin ymax zmin zmax]
...
hcb = colorbar;    % Poner la barra de colores
axis equal         % Hacer un grafico con coordenadas de igual tamaño
title({'Curvas de nivel de la función de tension de Prandtl \phi',...
   'Se muestran ademas los esfuerzos tangenciales',},'FontSize', 18) % Escribir el titulo
% Defino las coordenadas que quiero visualizar [xmin xmax ymin ymax]
axis([1.1*x(1) 1.1*x(end) 1.1*y(1) 1.1*y(end)]);
% Dibujo los bordes de la figura
plot(borde(:,1), borde(:,2), 'k-', 'LineWidth',5)
% Dibujo los esfuerzos cortantes
quiver(xx,yy,txz,tyz);
