...
archivo = 'ejemplo_C_clase_nov_2017.xls';
...
sx  = xlsread(archivo, 'airy', 'sx');  sx = flipud(sx);
sy  = xlsread(archivo, 'airy', 'sy');  sy = flipud(sy);
txy = xlsread(archivo, 'airy', 'txy'); txy = flipud(txy);
sz  = zeros(ny,nx);
txz = zeros(ny,nx);
tyz = zeros(ny,nx);
